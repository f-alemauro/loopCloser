...
unsigned int Loop_Closer::readDB() throw (runtime_error) {
	...
	if (myfile_1){
		...
		std::string lineData;
		int imgN;
		while(getline(myfile_1, lineData))
		{
	...
	if (myfile_2){
		...
		int imgN;
		getline(myfile_2, lineData);//column names
		while(getline(myfile_2, lineData))
		{
			std::stringstream lineStream(lineData);
			lineStream >> imgN;
			lineStream >> ts;
