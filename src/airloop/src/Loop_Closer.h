...
public:
    Loop_Closer(){}
    std::vector<std::vector<float> > calcProbabCamera(Vocabulary &VocabularyObject,flann::GenericIndex<flann::L2<float> >  &flannIndObj, const unsigned neighbour, const float lc_threshold, const unsigned subset_range, const float gmtr_threshold, Mat &image, unsigned int img_n) throw (runtime_error);
